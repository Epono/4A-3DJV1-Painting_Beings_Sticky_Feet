...
{
	#region PluginInfo
	[PluginInfo(Name = "InitPopulation", Category = "EX9.Texture", Help = "Basic template which creates a texture", Tags = "")]
	#endregion PluginInfo
	public class EX9_TextureInitPopulationNode : IPluginEvaluate, IPartImportsSatisfiedNotification
	{
		...
		unsafe void FillTexure(uint* data, int row, int col, int width, int height, Info info)
		{
			...
			var pixel = UInt32Utils.fromARGB(255, (byte)r.Next(), (byte)r.Next(), (byte)r.Next());
