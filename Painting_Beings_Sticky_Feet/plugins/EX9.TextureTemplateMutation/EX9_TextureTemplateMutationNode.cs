...
{
	#region PluginInfo
	[PluginInfo(Name = "TemplateMutation", Category = "EX9.Texture", Help = "Basic template which creates a texture", Tags = "")]
	#endregion PluginInfo
	public class EX9_TextureTemplateMutationNode : IPluginEvaluate, IPartImportsSatisfiedNotification
	{
		...
		[Input("Spreads Count", DefaultValue = 1)]
		public ISpread<int> FSpreadsCountIn;
		
		[Input("Colors", DefaultValue = 1)]
		public ISpread<Color4> FColorsIn;
		...
		public void Evaluate(int spreadMax)
		{
			...
			for (int i = 0; i < FSpreadsCountIn[0]; i++) {
				...
				if (info.SpreadsCount != FSpreadsCountIn[i]) {
					info.SpreadsCount = FSpreadsCountIn[i];
